...
void Migrate() {
	...
	for (auto s : serverRunVms) {
		serverRunVmsTmp.insert(make_pair(s.second, s.first)); // 按照虚拟机数量进行排序
		vmSum += s.second;
	}
	...
	while (head != tail) {
		if (moveNum > (vmSum / 200) || (vmSum / 200) == 0)
			break;
		while (head != tail && (*head).first != 0) {
			...
			for (auto sov = ServerOnVm[(*head).second].begin(); sov != ServerOnVm[(*head).second].end();) { // 遍历该服务器ID上的所存的vm
