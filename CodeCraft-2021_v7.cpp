...
void GetBestServer(int day, unordered_map<string, ServerInfo> serverinfos) {
	...
	for (auto s : serverinfos) {
		...
		PerformanceAndCostRatio.insert(make_pair(serverType, (ServerCost + day * PowerCost) / (Cpu + Memory)));  //根据key值可以自动排序
	}
...
void GetServerInfosPatch(unordered_map<string, ServerInfo>& serverinfos, unordered_map<string, VmInfo> vminfos, vector<vector<string>> requestinfos) {
	...
	vector<vector<vector<string>>> PatchRequestinfosTwoNode;
	vector<vector<vector<string>>> PatchRequestinfosOneNode;
	if (vminfostmp.size() > 100000) {   // 当虚拟机add请求数量很大时，分的更细

		PatchServerinfos.resize(PatchSize);

		for (auto s : serverinfostmp) {
			if (s.first <= 0.5) {   // 此参数可以修改（C语言老师看了想杀我系列）
		...
		// 对虚拟机请求按照双节点优先进行重排列，尽可能让双节点先分配，是否可行？
		PatchRequestinfosTwoNode.resize(PatchSize);
		PatchRequestinfosOneNode.resize(PatchSize);
		for (int i = 0; i < PatchSize; i++) {
			for (auto pre : PatchRequestinfos[i]) {

				if (VmInfos[pre[1]].VmTwoNodes == 1) {
					PatchRequestinfosTwoNode[i].push_back(pre);
				}
				else {
					PatchRequestinfosOneNode[i].push_back(pre);
				}
			}
			PatchRequestinfos[i].resize(PatchRequestinfosTwoNode[i].size());
			PatchRequestinfos[i] = PatchRequestinfosTwoNode[i];

			for (int j = 0; j < PatchRequestinfosOneNode[i].size(); j++) {
				for (int j = 0; j < PatchRequestinfosOneNode[i].size(); j++) {
					PatchRequestinfos[i].push_back(PatchRequestinfosOneNode[i][j]);
				}
		...
		// 对虚拟机请求按照双节点优先进行重排列，尽可能让双节点先分配，是否可行？  (不行！！！)
		PatchRequestinfosTwoNode.resize(2);
		PatchRequestinfosOneNode.resize(2);
		for (int i = 0; i < 2; i++) {
			for (auto pre : PatchRequestinfos[i]) {
