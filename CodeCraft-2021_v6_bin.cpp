...
int DealAddVm(vector<string> createVmInfo, vector<string>& res_tmp) {
	...
	// 换个思路，将差值与比值相结合
	multimap<int, pair<int, int>> serverDiffMap;  // <差值, <服务器id, 虚拟机部署方式>>  
	int vmCores = vm.CpuCores, vmMemo = vm.MemerySize, vmNode = vm.VmTwoNodes;
	if (vmNode == 0) {  // 单节点部署
		for (int i = 0; i < serverId; ++i) {
			if (sysServerResource[i].MemerySize_A != 0 && sysServerResource[i].CpuCores_A != 0) {
				int diff1 = sysServerResource[i].CpuCores_A - vmCores, diff2 = sysServerResource[i].MemerySize_A - vmMemo;
				// 写差值的映射
				if (diff1 >= 0 && diff2 >= 0) {
					serverDiffMap.insert(make_pair(diff1 + diff2, make_pair(i, 1)));
				}
			}

			if (sysServerResource[i].MemerySize_B != 0 && sysServerResource[i].CpuCores_B != 0) {
				int diff1 = sysServerResource[i].CpuCores_B - vmCores, diff2 = sysServerResource[i].MemerySize_B - vmMemo;
				// 写差值的映射
				if (diff1 >= 0 && diff2 >= 0) {
					serverDiffMap.insert(make_pair(diff1 + diff2, make_pair(i, 2)));
				}
			}
		}
	}
		...
		for (int i = 0; i < serverId; ++i) {
			if (sysServerResource[i].CpuCores_A != 0 && sysServerResource[i].MemerySize_A != 0 && sysServerResource[i].CpuCores_B != 0 && sysServerResource[i].MemerySize_B != 0) {
				// 找出和哪台服务器的差值最小
				...
				if (diff1A >= 0 && diff2A >= 0 && diff1B >= 0 && diff2B >= 0) {
					serverDiffMap.insert(make_pair(diff1A + diff2A + diff1B + diff2B, make_pair(i, 0)));
				}
			}
		}
	}
	// 在从某个差值范围中选出和该虚拟机内核比最接近的服务器
	if (serverDiffMap.empty())
		return -1;
	int minServerId = serverDiffMap.begin()->second.first, node = serverDiffMap.begin()->second.second, range = serverDiffMap.begin()->first;
	float minRatioDiff = 10000000.0;
	if (vm.VmTwoNodes) {  // 双节点部署
		float ratioDiff1 = abs((float)sysServerResource[minServerId].CpuCores_A / sysServerResource[minServerId].MemerySize_A - (float)(vm.CpuCores / 2) / (vm.MemerySize / 2));
		float ratioDiff2 = abs((float)sysServerResource[minServerId].CpuCores_B / sysServerResource[minServerId].MemerySize_B - (float)(vm.CpuCores / 2) / (vm.MemerySize / 2));
		minRatioDiff = ratioDiff1 + ratioDiff2;

		for (auto s : serverDiffMap) {
			if (s.first >= range + DIFF_RATIO_PARA)
				break;
			float ratioDiff1 = abs((float)sysServerResource[s.second.first].CpuCores_A / sysServerResource[s.second.first].MemerySize_A - (float)(vm.CpuCores / 2) / (vm.MemerySize / 2));
			float ratioDiff2 = abs((float)sysServerResource[s.second.first].CpuCores_B / sysServerResource[s.second.first].MemerySize_B - (float)(vm.CpuCores / 2) / (vm.MemerySize / 2));
			float ratioDiff = ratioDiff1 + ratioDiff2;
			if (minRatioDiff > ratioDiff) {
				minRatioDiff = ratioDiff;
				minServerId = s.second.first;
				node = s.second.second;
			}
		}
	}
	else {
		minRatioDiff = abs((float)sysServerResource[minServerId].CpuCores_A / sysServerResource[minServerId].MemerySize_A - (float)vm.CpuCores / vm.MemerySize);

		for (auto s : serverDiffMap) {
			if (s.first >= range + DIFF_RATIO_PARA)
				break;

			if (s.second.second == 1) { // 部署在A节点
				float ratioDiff = abs((float)sysServerResource[s.second.first].CpuCores_A / sysServerResource[s.second.first].MemerySize_A - (float)vm.CpuCores / vm.MemerySize);
				if (minRatioDiff > ratioDiff) {
					minRatioDiff = ratioDiff;
					minServerId = s.second.first;
					node = s.second.second;
			...
			else if (s.second.second == 2) { // 部署在B节点
				float ratioDiff = abs((float)sysServerResource[s.second.first].CpuCores_B / sysServerResource[s.second.first].MemerySize_B - (float)vm.CpuCores / vm.MemerySize);
				if (minRatioDiff > ratioDiff) {
					minRatioDiff = ratioDiff;
					minServerId = s.second.first;
					node = s.second.second;
