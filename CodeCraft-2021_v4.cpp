...
//set<string> ServerTypeNum;   // 每天需要清除信息
//multimap<string, ServerInfo> sysServerResourceTemp;
//// 扩容服务器 + 分配
//void Expansion(int day, unordered_map<string, ServerInfo> serverinfos, vector<vector<string>> requestinfos, multimap<string, unordered_map<string, vector<int>>>& ServerOnVm_type) {
//
//
...
//	// -----------------------------------------请求数据切分------------------------------------------- //
//	int NumBuffer = 1;
//	vector<vector<vector<string>>> requestinfos_buff;  // 切分的数据buff
//
//	if (requestinfos.size() < buff_length) {   // 如果数据小于bufflength，就不用切分
//		requestinfos_buff.push_back(requestinfos);
//	}
//	else {  
//	    NumBuffer = requestinfos.size() / buff_length + 1;
//		requestinfos_buff.resize(NumBuffer);
...
//			else {
//				vector<vector<string>>::iterator first = requestinfos.begin();
//				vector<vector<string>>::iterator second = requestinfos.begin() + buff_length;
//
//				requestinfos_buff[i].assign(first, second);
//				requestinfos.erase(requestinfos.begin(), requestinfos.begin() + buff_length);
//			}
//		}
//
//	}
...
//// -------------------------------------------开始数据批处理--------------------------------------------------- //
//
//	for (int patch = 0; patch < NumBuffer; patch++) {        
//
//		if (day == 0)  // 第一天我们全买，后面也可以调整day的大小
//		{
//			Expansion(day, serverinfos, requestinfos_buff[patch], ServerOnVm_type);
//			for (vector<vector<string>>::iterator req = requestinfos_buff[patch].begin(); req != requestinfos_buff[patch].end(); req++) {
...
//		}
//		else {
//			for (vector<vector<string>>::iterator req = requestinfos_buff[patch].begin(); req != requestinfos_buff[patch].end(); ) {
//				int opType = (*req).size() == 3 ? 1 : 0;
//				if (opType) {  // add
//
//					int resourceEnough = DealAddVm(*req, res_tmp);
//					if (resourceEnough != -1) {
//						req = requestinfos_buff[patch].erase(req);
//					}
//					else
//					{
//						req++;
